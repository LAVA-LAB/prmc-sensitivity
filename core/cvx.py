...
class cvx_verification_gurobi:
    ...
    def check_complementary_slackness(self, M):
        ...
        for i,(s,a) in enumerate(self.alpha.keys()):
            ...
            if M.is_sink_state(s):
                print('SINK STATE:', lambda_zero, alpha_zero)
...
class cvx_verification:
    ...
    def check_complementary_slackness(self, M):
        ...
        self.cns_dual = [self.cns[s].dual_value for s in range(len(M.states))]
        ...
        for i,(s,a) in enumerate(self.alpha.keys()):
            # if ('ineq', s, a) in self.cns:
                ...
                self.alpha_dual[(s, a)] = self.cns[('ineq', s, a)].dual_value
                self.alpha_primal[(s, a)] = self.alpha[(s, a)].value
