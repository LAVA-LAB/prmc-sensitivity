...
def parameter_importance_exp_visits(pmc, prmc, inst, CVX_GRB):
    ...
    instantiated_model, _ = pmc_instantiate(pmc, inst['valuation'])
    ...
    for s in instantiated_model.states:
        for a in s.actions:
            ...
            # If state is either a sink state or is nonrobust, then skip
            
            if instantiated_model.is_sink_state(s):
                continue
            
            isrobust = prmc.states_dict[s.id].actions_dict[a.id].robust
            if not isrobust:
                continue
